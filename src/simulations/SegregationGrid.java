...
public class SegregationGrid extends CA {
	...
	public SegregationGrid(XMLArgs xmlArgs, AutomatonDisplay a) {
		...
		simGrid = new FiniteGrid(getNumRow(), getNumCol(), 1, (int)getCellWidth(), Direction.ALL_DIRECTIONS);
	}
	...
	public void initializeScreen() {
		...
		for(Slot s : simGrid.getSlots()){
			...
			if (rand < vacantP) {
				SegregationSlotCell cell = new SegregationSlotCell(Color.GRAY, tPercentage, 0);
				s.addOccupant(cell);
			} else if (rand > vacantP && rand < vacantP + typeOne) {
				SegregationSlotCell cell = new SegregationSlotCell(Color.RED, tPercentage, 1);
				s.addOccupant(cell);
			} else {
				SegregationSlotCell cell = new SegregationSlotCell(Color.BLUE, tPercentage, 2);
				s.addOccupant(cell);
			}
	...
	public void updateCells() {
		...
		for(Slot s : simGrid.getSlots()){
			...
			SegregationSlotCell resident = (SegregationSlotCell)occupant.get(0);
			if(resident.cellUnsatisfied(cells)
					}
